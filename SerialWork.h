...
struct ParseBuffer
{
    ...
    uchar bufHead[2] = {0};
    uchar bufMsgid[2] = {0};
    QVector <uchar > bufBody;
    QVector <uchar > bufExt;
    uchar bufCRC[2] = {0};
};
...
public:
    ...
    ~SerialWork();
    int processData(uchar command, uchar subCommand, QByteArray data);
    bool getSerialIsoOpen();
    ...
    uint16_t crc16(const void* buf, int len)
    {
        ...
        while (len-- > 0) {
            r = (_crc_table[(r & 0xFF) ^ (*ptr++)]) ^ ((r >> 8) & 0xFF);
        }
        return r;
    }

signals:
    void signalFlashData(QString,int);
    void signaSerialportStateChanged(bool state);
    void signaShowMesgbox(QString);
    void signaShowMegDevStatus(QString);
    void signaShowMesgboxDialog(QString);
    void signalInitUIData(bool isOpen);
    void signalShowVolumeCurrNum(uchar leftChannel, uchar rightChannel,int nShowType);
    void signalShowBar(int maxValue, int Value);
    void signalContinueWriteData(uchar ret);
    void signalOperationResult(uchar cmd, uchar subCmd,uchar result);
    void signalShowCfgCmdReadData(int nReadMode,QVector <uchar > &boyData);
    void signalShowAlgCmdReadData(int nReadMode,QVector <uchar > &boyData);
    void signalShowUpdateCmdReadData(int nReadMode,QVector <uchar > &boyData);

    void signalCMDWriteFinshed(int size, int nBarType);
    void signalCMDAllFinshed();

    void signaleStartTimer();
    void signaleStopTimer();

    void signalShowNoOper();
    void signalShowChannelCountDefault();
public slots:
    void slotFlashReadToUiOrFile(int);
    void slotInitSerial(QString port, int baudRate);
    void slotCloseSerial();

private:
    SerialWork();
    SerialWork(const SerialWork&) = delete;
    SerialWork& operator=(const SerialWork&) = delete;

    void parseData(QByteArray data);
    void saveData(ParseBuffer,ushort msgid);
    QString numToString(QVector<uchar> &,int);
...
public:
    QSerialPort *m_pSerialIo = nullptr;
private:
    ushort m_msgid = -2;
    ushort m_sSerialReturnMsgID = 0;
    bool m_isSendData = true;
    ParseBuffer m_parseBuf;
    int m_flashReadToUiOrFile = 0;
    QMap<uchar, QString> m_errMap;
    QTimer *m_pTimeOutTimer = nullptr;
private:
    static QMutex mutexQueue;
    QQueue<QByteArray> m_qCommandQueue;
    bool m_bWaitingForResponse = false;
    ushort m_sSendMsgID = 0;
    QByteArray m_arraySendCommand;
};
